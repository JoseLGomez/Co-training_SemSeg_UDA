...
class DeepLabV3Head(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.loss_class_weight = cfg.MODEL.SEM_SEG_HEAD.LOSS_CLASS_WEIGHT
        self.loss_type        = cfg.MODEL.SEM_SEG_HEAD.LOSS_TYPE
        train_crop_size       = cfg.INPUT.CROP.SIZE
        aspp_dropout          = cfg.MODEL.SEM_SEG_HEAD.ASPP_DROPOUT
        use_depthwise_separable_conv = cfg.MODEL.SEM_SEG_HEAD.USE_DEPTHWISE_SEPARABLE_CONV
        self.batch_composition = cfg.SOLVER.SOURCE_PSEUDOLABELS_BATCH_RATIO
        self.loss_class_balance = None
        apply_validation_dropout = cfg.MODEL.SEM_SEG_HEAD.VALIDATION_DROPOUT
        # fmt: on
        ...
        if cfg.INPUT.CROP.ENABLED:
            ...
            if train_crop_h % self.common_stride or train_crop_w % self.common_stride:
                raise ValueError("Crop size %d, %d need to be divisible by output stride %d." %
                                 (train_crop_h, train_crop_w, self.common_stride))
        ...
        self.aspp = ASPP(
            ...
            dropout=aspp_dropout,
            use_depthwise_separable_conv=use_depthwise_separable_conv,
            validation_dropout=apply_validation_dropout
        )

        self.predictor = Conv2d(conv_dims, num_classes, kernel_size=1, stride=1, padding=0)
        nn.init.normal_(self.predictor.weight, 0, 0.001)
        nn.init.constant_(self.predictor.bias, 0)
        if self.loss_class_weight is not None:
            self.loss_class_weight = torch.FloatTensor(self.loss_class_weight)

        if self.loss_type == "cross_entropy":
            self.loss = nn.CrossEntropyLoss(reduction="mean", ignore_index=self.ignore_value)
            self.loss2 = nn.CrossEntropyLoss(reduction="mean", ignore_index=self.ignore_value)
        elif self.loss_type == "hard_pixel_mining":
            self.loss = DeepLabCE(ignore_label=self.ignore_value, top_k_percent_pixels=0.2, weight=self.loss_class_weight)
            self.loss2 = DeepLabCE(ignore_label=self.ignore_value, top_k_percent_pixels=0.2, weight=self.loss_class_weight)
        else:
            raise ValueError("Unexpected loss type: %s" % self.loss_type)
    ...
    def forward(self, features, targets=None, masks=None):
        ...
        if self.training:
            if masks is not None:
                return None, self.losses(x, targets, masks, weights=self.loss_class_balance)
            else:
                return None, self.losses(x, targets, weights=self.loss_class_balance)
        else:
            x = F.interpolate(
    ...
    def losses(self, predictions, targets, masks=None, weights=None):
        predictions = F.interpolate(
            predictions, scale_factor=self.common_stride, mode="bilinear", align_corners=False
        )
        if masks is not None:
        ...
        if weights is not None:
            if self.batch_composition is not None:
                loss1 = self.loss(predictions[:self.batch_composition[0]], targets[:self.batch_composition[0]])
                loss2 = self.loss2(predictions[-self.batch_composition[1]:], targets[-self.batch_composition[1]:])
                loss = loss1 + loss2
            else:
    ...
    def set_weights(self, cfg, weights):
        self.loss_class_balance = torch.FloatTensor(weights).to(torch.device(cfg.MODEL.DEVICE))
        if self.loss_type == "cross_entropy":
            self.loss2 = nn.CrossEntropyLoss(reduction="mean", ignore_index=self.ignore_value, weight=self.loss_class_balance)
        elif self.loss_type == "hard_pixel_mining":
            self.loss2 = DeepLabCE(ignore_label=self.ignore_value, top_k_percent_pixels=0.2, weight=self.loss_class_balance)
